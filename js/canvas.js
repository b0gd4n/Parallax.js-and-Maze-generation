...
var worker = new Worker("js/generate-wilsons.js");
...
worker.addEventListener("message", function(event) {
  ...
  $("#wait").delay(1000).animate({ opacity: 0 }, 1000);
  $("#wait").delay(1000).remove();
  $("#parallaxContainer").delay(1000).css("display", "block");
  $("#parallaxContainer").delay(1000).animate({ opacity: 1 }, 1000);
  $("#myCanvas").delay(1000).animate({ opacity: 0.5 }, 1000);
  ...
  d3.timer(function() {
    for (var i = 0, done; i < 5 && !(done = flood()); ++i);
